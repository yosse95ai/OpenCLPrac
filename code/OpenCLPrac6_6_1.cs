...
partial class Program
{
    ...
    static int[] lowPass(int[] wav, int windowSize)
    {
        ...
        SetKernelAeg(kernel, 2, windowSize);

        IntPtr[] globalSize = { (IntPtr)((wav.Length / 2) - windowSize + 1) };
        EnqueueNDRangeKernel(queue, kernel, 1, globalSize);     // request execute kernel
